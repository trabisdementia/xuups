 ...
 *
 * @page_title			"Article Archives" - Year - Month - Module's name
 *
 * @template_name		publisher_archive.html
 *
 ...
 * @template_var	string  lang_storytotal	Text ""
 */
######################################################################
# Original version:
...
if ($fromyear != 0 && $frommonth != 0) {
	...
	if(is_array($storyarray) && $count>0) {
		foreach ($storyarray as $item) {
			...
			/*
			if($infotips>0) {
				$story['infotips'] = news_make_infotips($item->hometext());
				$htmltitle=' title="'.$story['infotips'].'"';
			}*/

	    	$story['title'] = "<a href='".XOOPS_URL.'/modules/publisher/category.php?categoryid='.$item->categoryid()."'>".$item->getCategoryName()."</a>: <a href='".$item->getItemUrl()."'".$htmltitle.">".$item->title()."</a>";
	    	...
	    	$story['mail_link'] = 'mailto:?subject='.sprintf(_CO_PUBLISHER_INTITEM, $xoopsConfig['sitename']).'&amp;body='.sprintf(_CO_PUBLISHER_INTITEMFOUND, $xoopsConfig['sitename']).':  '. $item->getItemUrl();
