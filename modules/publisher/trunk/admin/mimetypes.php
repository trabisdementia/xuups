...
/**
* $Id: mimetypes.php 331 2007-12-23 16:01:11Z malanciault $
* Module: Publisher
...
$aSortBy = array('mime_id' => _AM_PUB_MIME_ID, 'mime_name' => _AM_PUB_MIME_NAME, 'mime_ext' => _AM_PUB_MIME_EXT,
                 'mime_admin' => _AM_PUB_MIME_ADMIN, 'mime_user' => _AM_PUB_MIME_USER);
$aOrderBy = array('ASC' => _AM_PUB_TEXT_ASCENDING, 'DESC' => _AM_PUB_TEXT_DESCENDING);
$aLimitBy = array('10' => 10, '15' => 15, '20' => 20, '25' => 25, '50' => 50, '100' => 100);
$aSearchBy = array('mime_id' => _AM_PUB_MIME_ID, 'mime_name' => _AM_PUB_MIME_NAME, 'mime_ext' => _AM_PUB_MIME_EXT);
...
function add()
{
    ...
    if(!isset($_POST['add_mime'])){
        ...
        publisher_adminMenu(4, _AM_PUB_MIMETYPES);
        
        publisher_collapsableBar('mimemaddtable', 'mimeaddicon', _AM_PUB_MIME_ADD_TITLE);
        ...
        echo "<tr valign='top'>
                  <td class='head'>"._AM_PUB_MIME_USERF."</td>
                  <td class='even'>";
        ...
        echo "<tr valign='top'>
                  <td class='head'></td>
                  <td class='even'>
                      <input type='submit' name='add_mime' id='add_mime' value='"._AM_PUB_BUTTON_SUBMIT."' class='formButton' />
                      <input type='button' name='cancel' value='"._AM_PUB_BUTTON_CANCEL."' onclick='history.go(-1)' class='formButton' />
                  </td>
              </tr>";
        echo "</table></form>";
        // end of add form
        ...
        echo "<table width='100%' cellspacing='1' class='outer'>";
        echo "<tr><th colspan='2'>"._AM_PUB_MIME_FINDMIMETYPE."</th></tr>";
        
        echo "<tr class='foot'>
                  <td colspan='2'><input type='submit' name='find_mime' id='find_mime' value='"._AM_PUB_MIME_FINDIT."' class='formButton' /></td>
              </tr>";
        
        echo "</table></form>";
        
        publisher_close_collapsable('mimeaddtable', 'mimeaddicon');
        ...
        if (strlen(trim($mime_ext)) == 0) {
            $has_errors = true;
            $error['mime_ext'][] = _AM_PUB_VALID_ERR_MIME_EXT;
        }
        
        if (strlen(trim($mime_name)) == 0) {
            $has_errors = true;
            $error['mime_name'][] = _AM_PUB_VALID_ERR_MIME_NAME;
        }
        
        if (strlen(trim($mime_types)) == 0) {
            $has_errors = true;
            $error['mime_types'][] = _AM_PUB_VALID_ERR_MIME_TYPES;
        }
        ...
        if(!$hMime->insert($mimetype)){
            redirect_header(PUBLISHER_ADMIN_URL."/mimetypes.php?op=manage&limit=$limit&start=$start", 3, _AM_PUB_MESSAGE_ADD_MIME_ERROR);
        } else {
...
function delete()
{
    ...
    if(!isset($_REQUEST['id'])){
        redirect_header(PUBLISHER_ADMIN_URL."/mimetypes.php", 3, _AM_PUB_MESSAGE_NO_ID);
    } else {
        $mime_id = intval($_REQUEST['id']);
    }
    $mimetype =& $hMime->get($mime_id);     // Retrieve mimetype object
    if(!$hMime->delete($mimetype, true)){
        redirect_header(PUBLISHER_ADMIN_URL."/mimetypes.php?op=manage&id=$mime_id&limit=$limit&start=$start", 3, _AM_PUB_MESSAGE_DELETE_MIME_ERROR);
    } else {
        header("Location: ".PUBLISHER_ADMIN_URL."/mimetypes.php?op=manage&limit=$limit&start=$start");
    }
}
...
function edit()
{
    ...
    if(!isset($_POST['edit_mime'])){
        ...
        // Display header
        publisher_xoops_cp_header();
        publisher_adminMenu(4, _AM_PUB_MIMETYPES . " > " . _AM_PUB_BUTTON_EDIT);
        
        publisher_collapsableBar('mimemedittable', 'mimeediticon', _AM_PUB_MIME_EDIT_TITLE);
        ...
        echo "<table width='100%' cellspacing='1' class='outer'>";
        echo "<tr><th colspan='2'>"._AM_PUB_MIME_MODIFYF."</th></tr>";
        echo "<tr valign='top'>
                  <td class='head'>"._AM_PUB_MIME_EXTF."</td>
                  <td class='even'><input type='text' name='mime_ext' id='mime_ext' value='$mime_ext' size='5' /></td>
              </tr>";
        ...
        echo "<tr valign='top'>
                  <td class='head'></td>
                  <td class='even'>
                      <input type='submit' name='edit_mime' id='edit_mime' value='"._AM_PUB_BUTTON_UPDATE."' class='formButton' />
                      <input type='button' name='cancel' value='"._AM_PUB_BUTTON_CANCEL."' onclick='history.go(-1)' class='formButton' />
                  </td>
              </tr>";
        echo "</table></form>";
        // end of edit form
        publisher_close_collapsable('mimeedittable', 'mimeediticon');
        xoops_cp_footer();
    } else {
        $mime_admin = 0;
        $mime_user = 0;
        ...
        if(!$hMime->insert($mimetype, true)){
            redirect_header(PUBLISHER_ADMIN_URL."/mimetypes.php?op=edit&id=$mime_id", 3, _AM_PUB_MESSAGE_EDIT_MIME_ERROR);
        } else {
...
function manage()
{
    ...
    if(isset($_POST['deleteMimes'])){      
        ...
        if($hMime->deleteAll($crit)){
            ...
            redirect_header(PUBLISHER_ADMIN_URL."/mimetypes.php?limit=$limit&start=$start", 3, _AM_PUB_MESSAGE_DELETE_MIME_ERROR);
        }
    }
    if(isset($_POST['add_mime'])){
    ...
    publisher_xoops_cp_header();
    publisher_adminMenu(4, _AM_PUB_MIMETYPES);
    publisher_collapsableBar('mimemanagetable', 'mimemanageicon', _AM_PUB_MIME_MANAGE_TITLE, _AM_PUB_MIME_INFOTEXT);
    ...
    echo "<table>";
    echo "<tr>";
    echo "<td align='right'>". _AM_PUB_TEXT_SEARCH_BY . "</td>";
    ...
    echo "</select></td>";
    echo "<td align='right'>"._AM_PUB_TEXT_SEARCH_TEXT."</td>";
    echo "<td align='left'><input type='text' name='search_text' id='search_text' value='' /></td>";
    echo "<td><input type='submit' name='mime_search' id='mime_search' value='"._AM_PUB_BUTTON_SEARCH."' /></td>";
    ...
    echo "<tr><td align='right'>"._AM_PUB_TEXT_SORT_BY." 
                  <select name='sort'>";
                ...
                echo "</select>
                  <input type='submit' name='mime_sort' id='mime_sort' value='"._AM_PUB_BUTTON_SUBMIT."' />
              </td>
          </tr>";
    echo "</table>";
    echo "</td></tr>";
    echo "<tr><th colspan='6'>"._AM_PUB_MIME_MANAGE_TITLE."</th></tr>";
    echo "<tr class='head'>
              <td>"._AM_PUB_MIME_ID."</td>
              <td>"._AM_PUB_MIME_NAME."</td>
              <td>"._AM_PUB_MIME_EXT."</td>
              <td>"._AM_PUB_MIME_ADMIN."</td>
              <td>"._AM_PUB_MIME_USER."</td>
              <td>"._AM_PUB_MINDEX_ACTION."</td>
          </tr>";
    ...
    echo "<tr class='foot'>
              <td colspan='6' valign='top'>
                  <a href='http://www.filext.com' style='float: right' target='_blank'>"._AM_PUB_MIME_FINDMIMETYPE."</a>
                  <input type='checkbox' name='checkAllMimes' value='0' onclick='selectAll(this.form,\"mimes[]\",this.checked);' />
                  <input type='submit' name='deleteMimes' id='deleteMimes' value='"._AM_PUB_BUTTON_DELETE."' />
                  <input type='submit' name='add_mime' id='add_mime' value='"._AM_PUB_MIME_CREATEF."' class='formButton' />
              </td>
          </tr>";
    echo "</table>";
    echo "<div id='staff_nav'>".$nav->renderNav()."</div>";
    
    publisher_close_collapsable('mimemanagetable', 'mimemanageicon');
...
function search()
{
    ...
    publisher_xoops_cp_header();
    publisher_adminMenu(4, _AM_PUB_MIMETYPES . " > " . _AM_PUB_BUTTON_SEARCH);
    
    publisher_collapsableBar('mimemsearchtable', 'mimesearchicon', _AM_PUB_MIME_SEARCH);
    
    if(!isset($_REQUEST['mime_search'])){
        
        echo "<form action='mimetypes.php?op=search' method='post'>";
        echo "<table width='100%' cellspacing='1' class='outer'>";
        echo "<tr><th colspan='2'>"._AM_PUB_TEXT_SEARCH_MIME."</th></tr>";
        echo "<tr><td class='head' width='20%'>"._AM_PUB_TEXT_SEARCH_BY."</td>
                  <td class='even'>
        ...
        echo "<tr><td class='head'>"._AM_PUB_TEXT_SEARCH_TEXT."</td>
                  <td class='even'>
        ...
        echo "<tr class='foot'>
                  <td colspan='2'>
                      <input type='submit' name='mime_search' id='mime_search' value='"._AM_PUB_BUTTON_SEARCH."' />
                  </td>
              </tr>";
        echo "</table></form>";
    } else {
    ...
    echo "</td></tr>";
        if(count($mimetypes) > 0){
            echo "<tr><th colspan='6'>"._AM_PUB_TEXT_SEARCH_MIME."</th></tr>";
            echo "<tr class='head'>
                      <td>"._AM_PUB_MIME_ID."</td>
                      <td>"._AM_PUB_MIME_NAME."</td>
                      <td>"._AM_PUB_MIME_EXT."</td>
            ...
            echo "<tr><th>"._AM_PUB_TEXT_SEARCH_MIME."</th></tr>";
            echo "<tr class='even'>
                      <td>"._AM_PUB_TEXT_NO_RECORDS."</td>
                  </tr>";
        }
        echo "</table>";
        echo "<div id='pagenav'>".$nav->renderNav()."</div>";
    }
    publisher_close_collapsable('mimesearchtable', 'mimesearchicon');
    xoops_cp_footer();
}
