...
if ($op == 'start')
{
	...
	publisher_collapsableBar('wfsectionimport', 'wfsectionimporticon',  sprintf(_AM_PUB_IMPORT_FROM, $importFromModuleName), _AM_PUB_IMPORT_INFO);
	...
	if ($totalCat == 0)
	{
		echo "<span style=\"color: #567; margin: 3px 0 12px 0; font-size: small; display: block; \">" . _AM_PUB_IMPORT_NOCATSELECTED . "</span>";
	}
	else
	{
...
if ($op == 'go')
{
	publisher_xoops_cp_header();
	publisher_adminMenu(-1, _AM_PUB_IMPORT);
	publisher_collapsableBar('wfsectionimportgo', 'wfsectionimportgoicon',  sprintf(_AM_PUB_IMPORT_FROM, $importFromModuleName), _AM_PUB_IMPORT_RESULT);
	...
	while ($arrCat = $xoopsDB->fetchArray ($resultCat))
	{
		...
		{
			echo sprintf(_AM_PUB_IMPORT_CATEGORY_ERROR, $arrCat['title']) . "<br/>";
			continue;
		}
