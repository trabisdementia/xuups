...
function publisher_seo_genUrl($op, $id, $short_url="")
{
    ...
    if (defined('PUBLISHER_SEO_ENABLED'))
    {
        ...
        else if (PUBLISHER_SEO_ENABLED == 'path-info')
        {
            ...
            return XOOPS_URL."/modules/publisher/seo.php/${op}.${id}/${short_url}";
        }
        else
        {
    ...
    else
    {
       // generate classic url
        switch ($op) {
            case 'category':
               return XOOPS_URL."/modules/publisher/${op}.php?categoryid=${id}";
            case 'item':
            case 'print':
               return XOOPS_URL."/modules/publisher/${op}.php?itemid=${id}";
            default:
