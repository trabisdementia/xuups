 ...
 * @version         $Id: viewtopic.php 2175 2008-09-23 14:07:03Z phppp $
 */
include 'header.php';
...
if ( $topic_handler->getPermission($forum_obj, $topic_obj->getVar('topic_status'), "addpoll") ) {
    if (!$topic_obj->getVar('topic_haspoll')) {
    ...
    } elseif ($isadmin || (is_object($poll) && is_object($xoopsUser) && $xoopsUser->getVar("uid")==$poll->getVar("user_id") ) ) {
        $poll_edit = "";
        $poll_delete = "";
        $poll_restart = "";

        $adminpoll_actions = array();
        $link_string = XOOPS_URL . '/modules/' . $xoopsModule->getVar("dirname", "n") . '/polls.php?op=%s&amp;poll_id=' . $topic_obj->getVar('poll_id') . '&amp;topic_id=' . $topic_id;
        $adminpoll_actions['editpoll'] = array(
            "link"    => sprintf($link_string, "edit"), 
            "image"    => $poll_edit,
            "name"    => _MD_EDITPOLL);
        ...
        $adminpoll_actions['restartpoll'] = array(
            "link"    => sprintf($link_string, "restart"), 
            "image"    => $poll_restart,
