...
class NewbbDigestHandler extends XoopsPersistableObjectHandler
{
    ...
    function NewbbDigestHandler(&$db)
    {
        $this->XoopsPersistableObjectHandler($db, 'bb_digest', 'Digest', 'digest_id');
    }
    ...
    function &getAllDigests($start = 0, $perpage = 5)
    {
        if (empty($start)) {
            $start = 0;
        }
    ...
    function delete($digest)
    {
        ...
        if ($this->last_digest == $digest->getVar('digest_id')) return false; // It is not allowed to delete the last digest
