...
// constructor
	function yogurt_configs ($id=null)
	{
	...
	function getAllyogurt_configss($criteria=array(), $asobject=false, $sort="config_id", $order="ASC", $limit=0, $start=0)
	{
...
* 
* @param object $yogurt_configs reference to the {@link yogurt_configs} object
* @param bool $force
* @return bool FALSE if failed, TRUE if already present and unchanged or successful
*/
	function insert(&$yogurt_configs, $force = false) {
		Global $xoopsConfig;
		if (get_class($yogurt_configs) != 'yogurt_configs') {
				return false;
		}
		...
		if ($yogurt_configs->isNew()) {
			...
			$sql = sprintf($format, $this->db->prefix('yogurt_configs'),
			$config_id
			,$config_uid
			,$pictures
            ,$audio
	 ...
	 * 
	 * @param object $yogurt_configs reference to the yogurt_configs to delete
	 * @param bool $force
	 * @return bool FALSE if failed.
	 */
	function delete(&$yogurt_configs, $force = false)
	{
		if (get_class($yogurt_configs) != 'yogurt_configs') {
			return false;
		}
