...
$_pluginInfo=array(
	...
	class interia extends OpenInviter_Base
	{
	    ...
	    public $allowed_domains=array('interia.pl', 'poczta.fm', 'interia.eu', '1gb.pl', '2gb.pl', 'vip.interia.pl', 'serwus.pl', 'akcja.pl', 'czateria.pl', 'znajomi.pl');

	    public $debug_array=array('initial_get'=>'pocztaLoginForm',
			  				  'login_post'=>'<button id="adressT" class="act">Kontakty</button>',
			  				  'url_contact'=>'while(1)'
			  				  );
			  				  private $sid;
			  				  ...
			  				  public function getMyContacts()
			  				  {
			  				      ...
			  				      foreach ($data as $line=>$param)
			  				      {
			  				          foreach ($param as $x)
			  				          {
			  				              ...
			  				              if ($pos_mail != false) $contacts[$email] = $firstname . " " . $lastname;
			  				          }
			  				      }
