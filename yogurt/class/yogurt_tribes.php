...
// constructor
	function yogurt_tribes ($id=null)
	{
	...
	function getAllyogurt_tribess($criteria=array(), $asobject=false, $sort="tribe_id", $order="ASC", $limit=0, $start=0)
	{
...
* 
* @param object $yogurt_tribes reference to the {@link yogurt_tribes} object
* @param bool $force
* @return bool FALSE if failed, TRUE if already present and unchanged or successful
*/
	function insert(&$yogurt_tribes, $force = false) {
		Global $xoopsConfig;
		if (get_class($yogurt_tribes) != 'yogurt_tribes') {
				return false;
		}
		...
		if ($yogurt_tribes->isNew()) {
			...
			$sql = sprintf($format, $this->db->prefix('yogurt_tribes'),
			$tribe_id
			,$owner_uid
			,$this->db->quoteString($tribe_title)
			,$this->db->quoteString($tribe_desc)
	 ...
	 * 
	 * @param object $yogurt_tribes reference to the yogurt_tribes to delete
	 * @param bool $force
	 * @return bool FALSE if failed.
	 */
	function delete(&$yogurt_tribes, $force = false)
	{
		if (get_class($yogurt_tribes) != 'yogurt_tribes') {
			return false;
		}
