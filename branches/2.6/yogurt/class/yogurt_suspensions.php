...
// constructor
	function yogurt_suspensions ($id=null)
	{
	...
	function getAllyogurt_suspensionss($criteria=array(), $asobject=false, $sort="uid", $order="ASC", $limit=0, $start=0)
	{
...
class Xoopsyogurt_suspensionsHandler extends XoopsObjectHandler
{
	...
	* @return object yogurt_suspensions
	*/
...
* 
* @param object $yogurt_suspensions reference to the {@link yogurt_suspensions} object
* @param bool $force
* @return bool FALSE if failed, TRUE if already present and unchanged or successful
*/
	function insert(&$yogurt_suspensions, $force = false) {
		Global $xoopsConfig;
		...
		if ($yogurt_suspensions->isNew()) {
			...
			$format .=" WHERE uid = %u";
			$sql = sprintf($format, $this->db->prefix('yogurt_suspensions'),
			$uid
			,$this->db->quoteString($old_pass)
			,$this->db->quoteString($old_email)
			,$this->db->quoteString($old_signature)
	 ...
	 * 
	 * @param object $yogurt_suspensions reference to the yogurt_suspensions to delete
	 * @param bool $force
