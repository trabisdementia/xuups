...
if ( isset($_POST['submit']) ) {
	...
	if ('delete' == $mode) {  
		foreach ($topic_id as $tid) {
            ...
            $forum = intval($topic_obj->getVar('forum_id'));
        ...
        $topics_obj =& $topic_handler->getAll($criteria);       
		...
		foreach ($topic_id as $tid) {
            ...
            if ($poll_id>0) {
                ...
                } elseif (is_dir(XOOPS_ROOT_PATH."/modules/umfrage/")){
        ...
        if ($newforum > 0) {
            ...
            foreach ($topic_id as $tid)
            {
                ...
                $sql = sprintf("UPDATE %s SET forum_id = %u WHERE topic_id = %u", $xoopsDB->prefix("bb_posts"), $newforum, $tid);
                if ( !$r = $xoopsDB->query($sql) ) {    return false;    }
            }
			$forum_handler->synchronization($newforum);
			$forum_handler->synchronization($forum);
            
        	echo $action[$mode]['msg']."<p><a href='viewtopic.php?forum=$newforum'>"._MD_GOTONEWFORUM."</a></p><p><a href='index.php'>"._MD_RETURNFORUMINDEX."</a></p>";
        } else {
            //DFK - 4.05
            SubmitForm($action, 'move',$topic_id, $forum);
        }
    } else {
        $topic_id = $topic_id[0];
    ...
    SubmitForm($action, $_GET['mode'], $topic_id, $forum);
}

function SubmitForm($action, $mode, $topic_id, $forum_id)
{
    ...
    //DFK - 4.05
    //loop $topic_id array and build an html input array
    foreach ($topic_id as $k => $v)
    {
        echo "<input type='hidden' name='topic_id[]' value='".$v."' />";
    }
