...
if ( $op == "add" ) {
    ...
    $barcolor_array = XoopsLists::getImgListAsArray(XOOPS_ROOT_PATH . "/modules/".$pollmodules."/images/colorbars/");
    for($i = 0; $i < 10; $i++) {
        ...
        $color_select->setExtra("onchange='showImgSelected(\"option_color_image[{$i}]\", \"option_color[{$i}]\", \"modules/".$pollmodules."/images/colorbars\", \"\", \"" . XOOPS_URL . "\")'");
        $color_label = new XoopsFormLabel("", "<img src='" . XOOPS_URL . "/modules/".$pollmodules."/images/colorbars/" . $current_bar . "' name='option_color_image[{$i}]' id='option_color_image[{$i}]' width='30' align='bottom' height='15' alt='' /><br />");
...
if ( $op == "save" ) {
    ...
    if ( $pollmodules == 'xoopspoll')
       $poll = new XoopsPoll();
    else if ( $pollmodules == 'umfrage')
        $poll = new Umfrage();
    else die("POLL-ERROR: polls.php Line 190");
    $poll->setVar("question", @$_POST['question']);
    $poll->setVar("description", @$_POST['description']);
    if ( !empty($_POST['end_time']) ) {
        $timezone = is_object($xoopsUser) ? $xoopsUser->getVar("timezone") : null;
    ...
    if ( !empty($new_poll_id) ) {
        ...
        foreach ( $option_text as $optxt ) {
            ...
            if ( $optxt != "" ) {
                ...
                else die("POLL-ERROR: polls.php Line 223");
...
if ( $op == "edit" ) {
    ...
    if ( $pollmodules == 'xoopspoll')
       $poll = new XoopsPoll($pollid);
    else if ( $pollmodules == 'umfrage')
        $poll = new Umfrage($pollid);
    ...
    foreach ($options_arr as $option) {
        ...
        $color_select->addOptionArray($barcolor_array);
        $color_select->setExtra("onchange='showImgSelected(\"option_color_image[{$i}]\", \"option_color[".$i."]\", \"modules/".$pollmodules."/images/colorbars\", \"\", \"" . XOOPS_URL . "\")'");
        $color_label = new XoopsFormLabel("", "<img src='" . XOOPS_URL . "/modules/".$pollmodules."/images/colorbars/" . $option->getVar("option_color", "E") . "' name='option_color_image[{$i}]' id='option_color_image[{$i}]' width='30' align='bottom' height='15' alt='' /><br />");
        $option_tray->addElement($color_select);
        $option_tray->addElement($color_label);
...
if ( $op == "savemore" ) {
    ...
    foreach ( $option_text as $optxt ) {
        $optxt = trim($optxt);
        if ( $optxt != "" ) {
            if ( $pollmodules == 'xoopspoll') $option = new XoopsPollOption();
            else $option = new UmfrageOption();
            $option->setVar("option_text", $optxt);
