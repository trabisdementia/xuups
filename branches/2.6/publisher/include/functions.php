...
function publisher_uploadFile($another = false, $withRedirect = true, &$itemObj)
{
    ...
    $session = $publisher->getHelper('session');
    $session->set('file_filename', isset($_POST['name']) ? $_POST['name'] : '');
    $session->set('file_description', isset($_POST['description']) ? $_POST['description'] : '');
    $session->set('file_status', isset($_POST['file_status']) ? intval($_POST['file_status']) : 1);
    $session->set('file_uid', $uid);
    $session->set('file_itemid', $itemid);
