...
function pathConfiguration()
{
    ...
    smartpartner_collapsableBar('configtable', 'configtableicon');
    echo "<img id='configtableicon' name='configtableicon' src=" . XOOPS_URL . "/modules/" . $xoopsModule->dirname() . "/images/icon/close12.gif alt='' /></a>&nbsp;" . _AM_SPARTNER_PATHCONFIGURATION . "</h3>";
...
if ((smartpartner_admin_getPathStatus('images', true) < 0)) {
    pathConfiguration();
}

// -- //
smartpartner_collapsableBar('toptable', 'toptableicon');
echo "<img id='toptableicon' name='toptableicon' src=" . XOOPS_URL . "/modules/" . $xoopsModule->dirname() . "/images/icon/close12.gif alt='' /></a>&nbsp;" . _AM_SPARTNER_INVENTORY . "</h3>";
...
echo "<img id='bottomtableicon' src=" . XOOPS_URL . "/modules/" . $xoopsModule->dirname() . "/images/icon/close12.gif alt='' /></a>&nbsp;" . _AM_SPARTNER_ALLITEMS . "</h3>";
echo "<div id='bottomtable'>";
echo "<span style=\"color: #567; margin: 3px 0 18px 0; font-size: small; display: block; \">" . _AM_SPARTNER_ALLITEMSMSG . "</span>";
...
$numrows = $partner_handler->getPartnerCount($statusSelected);
// creating the Q&As objects
$partnersObj = $partner_handler->getPartners($xoopsModuleConfig['perpage_admin'], $startentry, $statusSelected, $sortsel, $ordersel);
...
// Check Path Configuration
if ((smartpartner_admin_getPathStatus('images', true) > 0) && (smartpartner_admin_getPathStatus('images', true) > 0)) {
    pathConfiguration();
}
echo "</div>";
echo "</div>";
