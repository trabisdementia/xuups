...
class MycommentsCommentRenderer {
	...
	function renderFlatView($admin_view = false, $modlink = '', $itemlink = '')
	{
		...
		for ($i = 0; $i < $count; $i++) {
            ...
            $module =& $module_handler->get($this->_comments[$i]->getVar('com_modid'));

            $item = mycomments_plugin_execute( $module->getVar('dirname') , $this->_comments[$i]->getVar('com_itemid'), 'iteminfo');
            ...
            'module_link' => XOOPS_URL.'/modules/'.$module->getVar('dirname'),
            'module_name' => $module->getVar('name'),
            'editcomment_link' => XOOPS_URL.'/modules/'.$module->getVar('dirname').'/comment_edit.php?com_itemid='.$this->_comments[$i]->getVar('com_itemid'),
            'deletecomment_link' => XOOPS_URL.'/modules/'.$module->getVar('dirname').'/comment_delete.php?com_itemid='.$this->_comments[$i]->getVar('com_itemid'),
            'replycomment_link' => XOOPS_URL.'/modules/'.$module->getVar('dirname').'/comment_reply.php?com_itemid='.$this->_comments[$i]->getVar('com_itemid'),
            'poster' => $poster));
		}
	}
...
/*function renderThreadView($comment_id = 0, $admin_view = false, $show_nav = true)
	{
		...
		if (false != $show_nav && $tree[$comment_id]['obj']->getVar('com_pid') != 0) {
			$this->_tpl->assign('lang_top', _MA_MYCOM_TOP);
			$this->_tpl->assign('lang_parent', _MA_MYCOM_PARENT);
		...
		if (false != $admin_view) {
			// admins can see all
			$text = $this->_getText($tree[$comment_id]['obj']->getVar('com_text'),$tree[$comment_id]['obj']->getVar('com_pid')).'<div style="text-align:right; margin-top: 2px; margin-bottom: 0px; margin-right: 2px;">'._MA_MYCOM_STATUS.': '.$this->_statusText[$tree[$comment_id]['obj']->getVar('com_status')].'<br />IP: <span style="font-weight: bold;">'.$tree[$comment_id]['obj']->getVar('com_ip').'</span></div>';
		} else {
			// hide comments that are not active
			if (MYCOM_ACTIVE != $tree[$comment_id]['obj']->getVar('com_status')) {
...
/*	function _renderThreadReplies(&$thread, $key, &$replies, $prefix, $admin_view, $depth = 0, $current_prefix = '')
	{
        ...
        if (isset($thread[$key]['child']) && !empty($thread[$key]['child'])) {
            ...
            foreach ($thread[$key]['child'] as $childkey) {
                if (!$admin_view && $thread[$childkey]['obj']->getVar('com_status') != MYCOM_ACTIVE) {
...
/*	function _renderNestReplies(&$thread, $key, &$replies, $prefix, $admin_view, $depth = 0)
	{
        if ($depth > 0) {
			...
			$text = (false != $admin_view) ? $this->_getText($thread[$key]['obj']->getVar('com_text'),$thread[$key]['obj']->getVar('com_pid')).'<div style="text-align:right; margin-top: 2px; margin-right: 2px;">'._MA_MYCOM_STATUS.': '.$this->_statusText[$thread[$key]['obj']->getVar('com_status')].'<br />IP: <span style="font-weight: bold;">'.$thread[$key]['obj']->getVar('com_ip').'</span></div>' : $this->_getText($thread[$key]['obj']->getVar('com_text'),$thread[$key]['obj']->getVar('com_pid'));
