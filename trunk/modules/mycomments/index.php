...
//For the navbar get all modules
$mod_count=0;
$mod_list=array();
$modules =& $module_handler->getObjects(new Criteria('hascomments', 1), true);
foreach( $modules as $moduleid => $module ) {
    $items = array();
        //$items = comments_plugin_execute( $module->getVar('dirname') , $uid, 'useritems');
    $items = mycomments_get_plugin_info( $module->getVar('dirname'), 'useritems');
    if (is_array($items) && count($items)>0){
        $mod_list[$mod_count]['name'] = $module->getVar('name');
        $mod_list[$mod_count]['mid'] = $module->getVar('mid');
        $mod_count++;
    }
    unset($items);
}
...
for ( $i = 0; $i < $mod_count; $i++) {
    $sel = '';
    if ($mod_list[$i]['mid'] == $mid) {
        $sel = ' selected="selected"';
    }
    $mod_options .= '<option value="'.$mod_list[$i]['mid'].'"'.$sel.'>'.$mod_list[$i]['name'].'</option>';
}
...
if ( $com_count > $limit ) {
		...
		//if(publisher_isbot()) { 		// A bot is reading the news, we are going to show it all the links so that he can read everything
...
//$xoopsTpl->assign('topic_title',$xoopsModule->getVar('dirname'));
