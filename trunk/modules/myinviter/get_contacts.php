...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if ($step == 'get_contacts') {
        ...
        if (count($ers) == 0) {
            ...
            {
                ///hack here
                $contents .= "<script>self.close();</script>";
                $contents .= "<script>self.parent.location.reload();</script>";
                $contents .= "<div>Loading Contacts...</div>";
                @session_start();
                $_POST['oi_session_id'] = $inviter->plugin->getSessionID();
                $_SESSION['contacts_post'] = $_POST;
                echo $contents; exit;
    ...
    elseif ($step == 'send_invites')
    {
        ...
        if (count($ers) == 0) {
            ...
            {
                $internal = $inviter->getInternalError();
                $ers['internal'] = ($internal ? $internal
                        : "There were errors while sending your invites.<br>Please try again later!");
            }
            else {
...
$contents .= "<form method='POST' name='openinviter' action=''>" . ers($ers) . oks($oks);
if (!$done) {
    if ($step == 'get_contacts') {
...
if (!$done) {
    if ($step == 'send_invites') {
        if ($inviter->showContacts()) {
            $contents .= "<table align='center' cellspacing='0' cellpadding='0'><tr><td colspan='" . ($plugType == 'email'
                    ? "3" : "2") . "'>Your contacts</td></tr>";
            if (count($contacts) == 0) {
                $contents .= "<tr><td align='center' style='padding:20px;' colspan='" . ($plugType == 'email'
                        ? "3"
                        : "2") . "'>You do not have any contacts in your address book.</td></tr>";
            }
            else
            {
                $contents .= "<tr><td><input type='checkbox' onChange='toggleAll(this)' name='toggle_all' title='Select/Deselect all' checked>Invite?</td><td>Name</td>" . ($plugType == 'email'
                        ? "<td>E-mail</td>" : "") . "</tr>";
                $counter = 0;
                foreach ($contacts as $email => $name)
                {
                    $counter++;
                    $contents .= "<tr><td><input name='check_{$counter}' value='{$counter}' type='checkbox' checked><input type='hidden' name='email_{$counter}' value='{$email}'><input type='hidden' name='name_{$counter}' value='{$name}'></td><td>{$name}</td>" . ($plugType == 'email'
                            ? "<td>{$email}</td>" : "") . "</tr>";
                }
                $contents .= "<tr><td colspan='" . ($plugType == 'email' ? "3"
                        : "2") . "' style='padding:3px;'><input type='submit' name='send' value='Send invites'></td></tr>";
            }
            $contents .= "</table>";
        }
