...
// constructor
	function yogurt_friendship ($id=null)
	{
	...
	function getAllyogurt_friendships($criteria=array(), $asobject=false, $sort="friendship_id", $order="ASC", $limit=0, $start=0)
	{
...
class Xoopsyogurt_friendshipHandler extends XoopsObjectHandler
{
	...
	* @return object yogurt_friendship
	*/
...
* 
* @param object $yogurt_friendship reference to the {@link yogurt_friendship} object
* @param bool $force
* @return bool FALSE if failed, TRUE if already present and unchanged or successful
*/
	function insert(&$yogurt_friendship, $force = false) {
		Global $xoopsConfig;
		if (get_class($yogurt_friendship) != 'yogurt_friendship') {
				return false;
		}
		...
		if ($yogurt_friendship->isNew()) {
			...
			$sql = sprintf($format, $this->db->prefix('yogurt_friendship'),
			$friendship_id
			,$friend1_uid
			,$friend2_uid
			,$level
	 ...
	 * 
	 * @param object $yogurt_friendship reference to the yogurt_friendship to delete
	 * @param bool $force
	 * @return bool FALSE if failed.
	 */
	function delete(&$yogurt_friendship, $force = false)
	{
		if (get_class($yogurt_friendship) != 'yogurt_friendship') {
			return false;
		}
