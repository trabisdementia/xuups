...
// constructor
	...
	function getAllyogurt_ishots($criteria=array(), $asobject=false, $sort="cod_ishot", $order="ASC", $limit=0, $start=0)
	{
...
* 
* @param object $yogurt_ishot reference to the {@link yogurt_ishot} object
* @param bool $force
* @return bool FALSE if failed, TRUE if already present and unchanged or successful
*/
	function insert(&$yogurt_ishot, $force = false) {
		Global $xoopsConfig;
		if (get_class($yogurt_ishot) != 'yogurt_ishot') {
				return false;
		}
		...
		if ($yogurt_ishot->isNew()) {
			...
			$sql = sprintf($format, $this->db->prefix('yogurt_ishot'),
			$cod_ishot
			,$uid_voter
			,$uid_voted
			,$ishot
	 ...
	 * 
	 * @param object $yogurt_ishot reference to the yogurt_ishot to delete
	 * @param bool $force
	 * @return bool FALSE if failed.
	 */
	function delete(&$yogurt_ishot, $force = false)
	{
		if (get_class($yogurt_ishot) != 'yogurt_ishot') {
			return false;
		}
