...
// constructor
	function yogurt_reltribeuser ($id=null)
	{
	...
	function getAllyogurt_reltribeusers($criteria=array(), $asobject=false, $sort="rel_id", $order="ASC", $limit=0, $start=0)
	{
...
class Xoopsyogurt_reltribeuserHandler extends XoopsObjectHandler
{
	...
	* @return object yogurt_reltribeuser
	*/
...
* 
* @param object $yogurt_reltribeuser reference to the {@link yogurt_reltribeuser} object
* @param bool $force
* @return bool FALSE if failed, TRUE if already present and unchanged or successful
*/
	function insert(&$yogurt_reltribeuser, $force = false) {
		Global $xoopsConfig;
		if (get_class($yogurt_reltribeuser) != 'yogurt_reltribeuser') {
				return false;
		}
		...
		if ($yogurt_reltribeuser->isNew()) {
			...
			$format .= "VALUES (%u, %u, %u)";
			$sql = sprintf($format , 
			$this->db->prefix('yogurt_reltribeuser'), 
			$rel_id
			,$rel_tribe_id
			,$rel_user_uid
			);
			...
			$sql = sprintf($format, $this->db->prefix('yogurt_reltribeuser'),
			$rel_id
			,$rel_tribe_id
			,$rel_user_uid
			, $rel_id);
	 ...
	 * 
	 * @param object $yogurt_reltribeuser reference to the yogurt_reltribeuser to delete
	 * @param bool $force
	 * @return bool FALSE if failed.
	 */
	function delete(&$yogurt_reltribeuser, $force = false)
	{
		if (get_class($yogurt_reltribeuser) != 'yogurt_reltribeuser') {
			return false;
		}
