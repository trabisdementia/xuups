...
// constructor
	...
	function getAllyogurt_audios($criteria=array(), $asobject=false, $sort="audio_id", $order="ASC", $limit=0, $start=0)
	{
...
* 
* @param object $yogurt_audio reference to the {@link yogurt_audio} object
* @param bool $force
* @return bool FALSE if failed, TRUE if already present and unchanged or successful
*/
	function insert(&$yogurt_audio, $force = false) {
		Global $xoopsConfig;
		...
		if ($yogurt_audio->isNew()) {
			...
			$sql = sprintf($format, $this->db->prefix('yogurt_audio'),
			$audio_id
			,$this->db->quoteString($title)
			,$this->db->quoteString($author)
			,$this->db->quoteString($url)
	 ...
	 * 
	 * @param object $yogurt_audio reference to the yogurt_audio to delete
	 * @param bool $force
