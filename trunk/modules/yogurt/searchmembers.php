<?php
// $Id: searchmembers.php,v 1.4 2008/04/19 16:39:10 marcellobrandao Exp $
//  ------------------------------------------------------------------------ //
...
if($op == 'form')
{
	...
	$sort_select->addOptionArray(array('uname'=>_MD_YOGURT_UNAME,'email'=>_MD_YOGURT_EMAIL,'last_login'=>_MD_YOGURT_LASTLOGIN,'user_regdate'=>_MD_YOGURT_REGDATE,'posts'=>_MD_YOGURT_POSTS));
	$order_select = new XoopsFormSelect(_MD_YOGURT_ORDER, 'user_order');
	$order_select->addOptionArray(array('ASC'=>_MD_YOGURT_ASC,'DESC'=>_MD_YOGURT_DESC));
...
if($op == 'submit')
{
	$xoopsOption['template_main'] = 'yogurt_searchresults.html';
	include XOOPS_ROOT_PATH.'/header.php';
	...
	if($total == 0) {$xoopsTpl->assign('lang_nonefound', _MD_YOGURT_NOFOUND);}
	elseif($start < $total )
	{
