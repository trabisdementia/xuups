...
function add()
{
    ...
    if (!isset($_POST['add_mime'])) {
        ...
        $mime_type = $publisher->getHelper('session')->get('addMime');
        $mime_errors = $publisher->getHelper('session')->get('addMimeErr');
        ...
        if ($has_errors) {
            ...
            $publisher->getHelper('session')->set('addMime', $mime);
            $publisher->getHelper('session')->set('addMimeErr', $error);
...
function edit()
{
    ...
    if (!isset($_POST['edit_mime'])) {
        $mime_type = $publisher->getHelper('session')->get("editMime_{$mime_id}");
        $mime_errors = $publisher->getHelper('session')->get("editMimeErr_{$mime_id}");
        ...
        if ($has_errors) {
            ...
            $mime['mime_admin'] = $mime_admin;
            $mime['mime_user'] = $mime_user;
            $publisher->getHelper('session')->set('editMime_' . $mime_id, $mime);
            $publisher->getHelper('session')->set('editMimeErr_' . $mime_id, $error);
...
{
    $publisher =& Xmf_Module_Helper::getInstance(PUBLISHER_DIRNAME);
    $publisher->getHelper('session')->del('addMime');
    $publisher->getHelper('session')->del('addMimeErr');
}
...
function _clearEditSessionVars($id)
{
    $id = intval($id);
    $publisher =& Xmf_Module_Helper::getInstance(PUBLISHER_DIRNAME);
    $publisher->getHelper('session')->del("editMime_{$id}");
    $publisher->getHelper('session')->del("editMimeErr_{$id}");
}
